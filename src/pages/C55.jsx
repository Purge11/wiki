/* eslint-disable jsx-a11y/accessible-emoji */
import React from 'react'

export default function C55() {
    return (
        <div className='c55'>
            <div className="container d-flex justify-content-center">
                <div className="row series-image-list ">

                    <h4 className=" nosvoid-color d-flex justify-content-center">C55 Equipment Guide</h4>

                    <br />
                    <p className="d-flex justify-content-center">If you have become Prestige 7, you can teleport to the map by typing $warp p7.</p>
                    <br />

                    <br />
                    On p7 map you will find "Craft C55 Equipments NPC", where you can craft C55 Equipments.<br /><br />
                    <p className="d-flex justify-content-center">
                        <img src="asset/c55/c55npc.png" alt="" /><hr /><br />
                    </p>
                    This are the materials you'll need to craft c55 Equipment:
                    <br />

                    <br />
                    <p>
                        <b className="item-effect">- Gold Coin x500:</b> Drops from Prestige 7 monsters.<br />
                        <b className="item-effect">- Swordsman, Mage, Archer, MA Aura x5:</b> Drops from Pure Beast Raid Box<br />
                        <b className="item-effect">- Black Powder x2:</b> You can get it by doing raid daily quests of Kirollas. "(Can be done once a day)"<br />
                        <b className="item-effect">- Golden Powder x2:</b> You can get it by doing raid daily quests of Pure Beast. "(Can be done once a day)"<br />
                        <b className="item-effect">- Holy light x1:</b> Caligor raid box and Drops from Hardcore Raid Box "(Bounce rate is low.)"<br />
                        <b className="item-effect">- Ancient Equipment x3</b> (bow, sword, armour...): Drops from Kirollas Raid Box<br />
                        <b className="item-effect">- Equipment Crystal x7</b> (bow, sword, dagger...): Drops from Tower box.<br />
                        <b className="item-effect">- Flame Crystal x1:</b> Can be bought from Alchemist NPC on Nosville. (or $buy crystal X | X = Amount to buy) <br />
                        <b className="item-effect">- C50 Equipment:</b> Drops from Fernon Raids<br />
                    </p>

                    <br />

                    <div className="text-danger d-flex justify-content-center">Note: Don't forget to put the C50 Equipment in the first
                        slot of your inventory while crafting.</div>
                    <br /><br />

                    <img src="asset/c55/c55.png" alt="" />
                </div>
            </div>
        </div>
    )
}
